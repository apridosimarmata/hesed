# Threat Model - poimen MCP Security Sidecar

## System Overview

poimen is a reverse-proxy sidecar that intercepts JSON-RPC traffic between AI agents and MCP tool servers. Every `tools/call` request passes through a security pipeline:

```
Agent → [Interceptor → Breaker → AuthZ → DLP → HITL → Upstream Tool] → Agent
```

This document enumerates threats against each stage, their impact, and the mitigations poimen provides (or should provide).
...
|---|---|
| Agent ↔ poimen | The agent is semi-trusted - it follows instructions but may be manipulated via prompt injection |
| poimen ↔ Upstream MCP | The upstream tool server is untrusted - it may return sensitive data or behave unexpectedly |
| Config file ↔ poimen | The config is fully trusted - whoever controls `config.toml` controls all policy |
| HITL webhook ↔ poimen | The webhook endpoint is trusted to return honest approve/deny decisions |
...
**Mitigation:** poimen rejects any payload that fails `serde_json` deserialization with a JSON-RPC error response (`-32700 Parse error`). Non-tool-call methods are passed through but never touch the security pipeline.  
...
**Mitigation:** poimen only applies the pipeline to `tools/call` - all other methods are passed through by design. This is correct behavior per MCP spec.  
...
**Mitigation:** poimen extracts the role from the JSON-RPC `params` field. The role-to-tool mapping is defined in `config.toml` and cannot be overridden by the request itself. However, if the role value in the request is attacker-controlled, the attacker can claim any role.  
...
**Mitigation:** This is a configuration issue, not a code bug. poimen faithfully enforces whatever policy is configured.  
...
**Mitigation:** poimen applies regex patterns to the serialized JSON string of params/results. This catches plaintext PII but not encoded forms.  
...
**Mitigation:** Operators must tune DLP patterns carefully. poimen applies the configured `redact_replacement` string uniformly.  
...
**Threat:** A compromised or runaway agent floods poimen with requests, exhausting the token bucket and causing legitimate requests to be rate-limited.  
**Impact:** Denial of service for legitimate tool calls.  
**Mitigation:** poimen uses `governor` with a configurable `requests_per_second` and `burst_size`. Excess requests receive a JSON-RPC error (`-32000`).  
...
**Mitigation:** poimen sends a POST to the configured `webhook_url` and trusts the response. There is no signature verification or shared secret on the webhook response.  
...
**Mitigation:** poimen uses `reqwest` with a 30-second timeout. On timeout or error, the request is denied.  
...
**Mitigation:** poimen writes audit events but does not protect the sink itself. File-based logs have no integrity protection. Webhook-based sinks depend on the receiver's security.  
...
**Mitigation:** poimen reads config at startup and trusts it entirely. No runtime integrity checks.  
...
**Mitigation:** poimen applies DLP redaction to upstream responses before returning them to the agent. This catches plaintext PII but not encoded injection payloads.  
...
**Stage:** Agent ↔ poimen, poimen ↔ Upstream  
**Threat:** An attacker on the network intercepts plaintext traffic between the agent and poimen, or between poimen and the upstream tool server.  
**Impact:** Credential theft, PII exposure, request/response tampering.  
**Mitigation:** poimen listens on a local address (`127.0.0.1`) by default, limiting exposure. However, it does not terminate TLS - both the agent-facing and upstream connections are plaintext HTTP.  
...
**Stage:** Build  
**Threat:** A compromised crate in the dependency tree introduces malicious code into the poimen binary.  
